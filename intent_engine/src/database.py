...
_DDL = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_signals_phase    ON intent_signals(phase)",
    "CREATE INDEX IF NOT EXISTS idx_signals_product  ON intent_signals(detected_product)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_signals_scan_run ON intent_signals(scan_run_id)",
    ...
    CREATE TABLE IF NOT EXISTS technology_profiles (
        ...
        taxonomy_products      TEXT[] NOT NULL DEFAULT '{}',
        is_active            BOOLEAN NOT NULL DEFAULT TRUE,
        created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
    ...
    # content_hash dedup for intent_signals — prevents cross-source duplicates
    "ALTER TABLE intent_signals ADD COLUMN IF NOT EXISTS content_hash TEXT",
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_signals_hash ON intent_signals(content_hash) WHERE content_hash IS NOT NULL",
...
""",
    ...
    # intent_signals.scan_run_id and companies.first_scan_run_id were declared as
    ...
    """
    DO $$ BEGIN
        UPDATE intent_signals SET scan_run_id = NULL
        WHERE scan_run_id IS NOT NULL
          AND NOT EXISTS (SELECT 1 FROM scan_runs WHERE id = intent_signals.scan_run_id);
    ...
    with db_cursor(commit=False) as cur:
        cur.execute("""
            ...
            WHERE c.signal_count = 0
              AND EXISTS (SELECT 1 FROM intent_signals s WHERE s.company_id = c.id)
        """)
...
# Fix 4: detected_product normalization — maps free-text aliases to canonical names.
...
def insert_signal(company_id: int, detected_product: str, phase: str, source: str,
                  signal_type: str, job_title: str, evidence: str,
                  url: str, confidence: float, scan_run_id: int = None) -> int:
    detected_product = _normalize_oracle_product(detected_product)
    ...
    with db_cursor() as cur:
        ...
        cur.execute("""
            INSERT INTO intent_signals
                (company_id, scan_run_id, detected_product, phase, source,
    ...
    with db_cursor(commit=False) as cur:
        cur.execute("""
            SELECT DISTINCT ON (LOWER(c.name))
                   c.name, s.detected_product, s.phase, s.signal_type,
                   s.job_title, s.evidence, s.source, s.confidence
            FROM companies c
            JOIN intent_signals s ON s.company_id = c.id
        ...
        for r in cur.fetchall():
            bits = [b for b in (r["signal_type"], r["detected_product"], r["phase"]) if b]
        ...
        cur.execute(f"""
            ...
            LEFT JOIN (
                SELECT company_id,
                       ...
                       (ARRAY_AGG(detected_product ORDER BY confidence DESC NULLS LAST))[1] AS top_product
                FROM intent_signals
                GROUP BY company_id
...
def aggregate_product_intel() -> dict:
    """
    Read intent_signals grouped by company, classify products as Cloud or On-Premise,
    ...
    with db_cursor(commit=False) as cur:
        cur.execute("""
            SELECT company_id,
                   ...
                   COUNT(*)        AS signal_count
            FROM intent_signals
            WHERE detected_product IS NOT NULL AND detected_product <> ''
            GROUP BY company_id, detected_product
        """)
        rows = cur.fetchall()
        ...
        cur.execute("""
            ...
            FROM (
                SELECT DISTINCT ON (company_id)
                       company_id,
                       detected_product AS top_product
                FROM   intent_signals
                WHERE  detected_product IS NOT NULL
                  AND  detected_product NOT IN ('', 'Oracle (General)')
                GROUP  BY company_id, detected_product
...
def merge_companies(keep_id: int, drop_id: int) -> dict:
    ...
    with db_cursor() as cur:
        ...
        cur.execute("""
            UPDATE intent_signals SET company_id = %s
            WHERE company_id = %s
              AND url NOT IN (
                  SELECT url FROM intent_signals WHERE company_id = %s AND url IS NOT NULL
              )
        ...
        if run_id:
            ...
            cur.execute("""
                WITH sig AS (
                    SELECT company_id,
                           COUNT(*)                                  AS signal_count,
                           STRING_AGG(DISTINCT detected_product, ',')  AS products,
    ...
    with db_cursor(commit=False) as cur:
        cur.execute("""
            SELECT o.id AS outcome_id, o.outcome,
                   s.signal_type, s.detected_product, s.phase, s.source
            FROM outcomes o
            JOIN intent_signals s ON s.company_id = o.company_id
        """)
        return [dict(r) for r in cur.fetchall()]
