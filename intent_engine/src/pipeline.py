...
def run_scan(
    ...
    try:
        ...
        classified = [s for s in classified if s.get("detected_product")]
    ...
    for company in companies:
        ...
        try:
            ...
            for sig in company.get("signals", []):
                detected_product = sig.get("detected_product")
                if not detected_product or detected_product == "Oracle (General)":
