...
platform multi-product.  Each profile defines:
  ...
  - taxonomy_products[]   → product names matched by phase classifier
...
def create_profile(
    ...
    taxonomy_products: list = None,
) -> dict:
    with db.db_cursor() as cur:
        cur.execute(
            """INSERT INTO technology_profiles
                   (name, description, keywords, target_websites,
                    competitor_domains, partner_domains, manufacturer_domain, taxonomy_products)
