...
_DDL = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_signals_tier        ON intent_signals(signal_tier)",
    ...
    "ALTER TABLE intent_signals ADD COLUMN signal_tier TEXT DEFAULT 'P2'",
...
def reset_all_data():
    with db_cursor() as cur:
        for tbl in ["intent_signals", "company_contacts", "companies", "scan_runs"]:
...
def merge_companies(keep_id: int, drop_id: int) -> dict:
    with db_cursor() as cur:
        cur.execute("UPDATE intent_signals   SET company_id = ? WHERE company_id = ?", (keep_id, drop_id))
        ...
        cur.execute("""
            UPDATE companies SET
                signal_count  = (SELECT COUNT(*) FROM intent_signals   WHERE company_id = id),
...
def backfill_target_product() -> int:
    with db_cursor(commit=False) as cur:
        cur.execute("""
            SELECT c.id, GROUP_CONCAT(DISTINCT s.detected_product) AS products
            FROM companies c
            JOIN intent_signals s ON s.company_id = c.id
            WHERE c.target_product = '' AND s.detected_product != ''
            GROUP BY c.id
        """)
        rows = cur.fetchall()
    count = 0
        ...
        if run_id:
            cur.execute("""
                SELECT
                    ...
                    COALESCE(NULLIF(c.target_product, ''),
                             GROUP_CONCAT(DISTINCT s.detected_product)) AS target_product,
                    COUNT(s.id)                             AS signal_count,
                    GROUP_CONCAT(DISTINCT s.detected_product) AS products,
                    ...
                    (SELECT url FROM intent_signals
            ...
            cur.execute("""
                SELECT
                    ...
                    COALESCE(ct.contact_count, 0)           AS contact_count
                FROM companies c
                LEFT JOIN intent_signals s ON s.company_id = c.id
                LEFT JOIN (
                    SELECT company_id, COUNT(*) AS contact_count
...
def batch_update_signal_counts(company_ids: list) -> None:
    for cid in company_ids:
        with db_cursor() as cur:
            cur.execute("""
                UPDATE companies SET
                    signal_count = (SELECT COUNT(*) FROM intent_signals WHERE company_id = ?)
                WHERE id = ?
            """, (cid, cid))
...
def get_signals_for_company(company_id: int) -> list:
    with db_cursor(commit=False) as cur:
        cur.execute(
            "SELECT * FROM intent_signals WHERE company_id = ? ORDER BY detected_at DESC",
            (company_id,),
        )
        return cur.fetchall()
...
def get_top_signals_for_companies(names: list) -> dict:
    ...
    with db_cursor(commit=False) as cur:
        cur.execute(f"""
            SELECT c.name, s.detected_product, s.phase, s.signal_type,
...
def aggregate_product_intel() -> dict:
    with db_cursor(commit=False) as cur:
        cur.execute("""
            ...
            FROM intent_signals
            WHERE detected_product != '' AND detected_product IS NOT NULL
...
def get_signal_aggregates_by_company(company_ids: list) -> dict:
    ...
    with db_cursor(commit=False) as cur:
        cur.execute(f"""
            SELECT company_id,
                   GROUP_CONCAT(DISTINCT detected_product) AS products,
