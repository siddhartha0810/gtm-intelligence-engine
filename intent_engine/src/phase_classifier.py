...
Classifies a raw signal (job posting, news article) into:
  detected_product — which product/keyword is involved (Oracle taxonomy by
...
HOW IT FITS IN THE SYSTEM:
  Called by scan_worker.py for every raw signal returned by a scraper:
    ...
    2. classify()        → adds taxonomy_products[], phases[], confidence
    3. database.upsert_signal() → stored in intent_signals table
...
# icp_profiles/taxonomy_products.yaml — keep the two in sync if either changes.
_FALLBACK_YAML = Path(__file__).resolve().parent.parent.parent / "icp_profiles" / "taxonomy_products.yaml"
...
def classify(title: str, description: str, source: str = "",
    ...
    Returns:
        {detected_product, phase, phase_label, confidence}
    """
