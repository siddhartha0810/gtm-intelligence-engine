...
import {
  Briefcase, ExternalLink, Users, Building2, Radar, Landmark, ShieldCheck,
...
export default function Endex() {
  const [data, setData]       = useState<EndexData | null>(null)
  ...
  useEffect(() => {
    fetch('/api/decision-intelligence/endex', { headers: authH() })
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Landmark size={22} color={C.primary} />
          <h1 style={{ fontSize: 24, fontWeight: 700, color: C.text, margin: 0 }}>
            {icp.meta?.company || 'Endex'}
          </h1>
        </div>
        <p style={{ color: C.textMute, marginTop: 4 }}>
          {icp.meta?.domain} — target ICP, signal rules, and live pipeline for the Endex account
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div style={card}>
          ...
          <p style={{ fontSize: 12, color: C.textFaint, margin: '-6px 0 12px' }}>Endex's own logos — proof points for ICP fit, not prospects</p>
